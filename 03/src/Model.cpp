...
void Model::loadData(const std::string& filename)
{
    ...
    if(fp.is_open()) {
        ...
        while(fp.getline(line, linesize)) {
            if(std::string(line).length() > 1) {
                ...
                else
                {
                    ...
                    std::cout << "vector size: " << data_stl_vec.size() << std::endl;
                    for (auto i = data_stl_vec.begin(); i != data_stl_vec.end(); i++)
                    {
                        std::cout << *i << ", ";
                    }
                    std::cout << std::endl;
                    ...
                    // std::cout << "line length: " << std::string(line).length() << ", " << std::string(line) << std::endl;
        ...
        std::cout<< "update control section" << std::endl;
        updateSplineSections();
        std::cout<< "update spline section" << std::endl;
        fp.close();
    }
    else {
...
void Model::drawModel(void) const
{
    ...
    for (size_t i = 0; i < num_of_control_sections; i++)
    {
        const glm::vec3* spline_curve = control_sections[i].getSectionSplineCurve();
        size_t num_of_spline_points_per_section = control_sections[i].getNumOfSplinePoints();

        glBegin(GL_LINE_LOOP);
        for (size_t point_idx = 0; point_idx < num_of_spline_points_per_section; point_idx++)
        {
            glVertex3f(spline_curve[point_idx].x, spline_curve[point_idx].y, spline_curve[point_idx].z);
            glVertex3f(spline_curve[point_idx < num_of_spline_points_per_section-1 ? point_idx+1 : 0].x, spline_curve[point_idx < num_of_spline_points_per_section-1 ? point_idx+1 : 0].y, spline_curve[point_idx < num_of_spline_points_per_section-1 ? point_idx+1 : 0].z);  
        }
        glEnd();
    ...
    for (size_t i = 0; i < num_of_spline_sections; i++)
    {
        ...
        const glm::vec3* next_spline_curve = i < num_of_spline_sections-1 ? spline_sections[i+1].getSectionSplineCurve() : control_sections[num_of_control_sections-1].getSectionSplineCurve();
        size_t num_of_spline_points_per_section = spline_sections[i].getNumOfSplinePoints();
