...
void OffModel::generateFaceNormals(void)
{
    ...
    vertex_normals_list = new std::list<glm::vec3>[num_of_vertices];
    vertices_normals = new glm::vec3 [num_of_vertices];
    ...
    for (size_t face_idx = 0; face_idx < num_of_faces; face_idx++)
    {
        ...
        glm::vec3 face_normal = glm::vec3(0.0f, 0.0f, 0.0f);
        for (size_t i = 0; i < num_of_vertices_per_face; i++)
        {
            glm::vec3 cur_vertex = vertices[vertices_per_face[i]];
            glm::vec3 next_vertex = vertices[vertices_per_face[(i+1) % num_of_vertices_per_face]];
            glm::vec3 next_next_vertex = vertices[vertices_per_face[(i+2) % num_of_vertices_per_face]];

            glm::vec3 v1 = next_vertex - cur_vertex;
            glm::vec3 v2 = next_next_vertex - next_vertex;
        ...
        // glm::vec3 v0 = vertices[std::get<1>(faces[face_idx])[0]];
        // glm::vec3 v1 = vertices[std::get<1>(faces[face_idx])[1]];
        // glm::vec3 v2 = vertices[std::get<1>(faces[face_idx])[2]];
        // std::get<2>(faces[face_idx]) = -glm::normalize(glm::cross(v0-v1, v2-v1));
    }
    for (size_t i = 0; i < num_of_vertices; i++)
    {
        glm::vec3 sum = glm::vec3(0.0f, 0.0f, 0.0f);
        for (std::list<glm::vec3>::const_iterator iter = vertex_normals_list[i].begin() ; iter != vertex_normals_list[i].end(); iter++)
        {
            sum += *iter;
        }
...
void OffModel::draw(void) const
{
    ...
    for (size_t face_idx = 0; face_idx < num_of_faces; face_idx++)
    {
        ...
        for (size_t vertex_idx = 0; vertex_idx < num_of_vertex_per_face; vertex_idx++)
        {
            ...
            glm::vec3 vertex_normal = vertices_normals[vertices_array_per_face[vertex_idx]];
            glNormal3f(vertex_normal.x, vertex_normal.y, vertex_normal.z);
            glVertex3f(vertex_per_face.x, vertex_per_face.y, vertex_per_face.z);
        }
        glEnd();
    }
    glPopMatrix();
